...
def _load_prompt_template(prompt_name: str) -> PromptTemplate:
    ...
    if "template" not in prompt_data:
        raise ValueError(f"Prompt file missing 'template' field: {prompt_file}")
    ...
    if isinstance(input_variables, list) and len(input_variables) > 0:
        ...
        var_names = [var["name"] for var in input_variables if isinstance(var, dict)]
    else:
    ...
    logger.debug(f"Loaded prompt template '{prompt_data.get('name', prompt_name)}' with {len(template.input_variables)} variables")
