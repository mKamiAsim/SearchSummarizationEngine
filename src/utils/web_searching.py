...
def search_web(
    ...
    logger.info(f"Executing web search for: '{query}' (max {num_results} results)")
...
def search_multiple_queries(
    ...
    for idx, query in enumerate(queries):
        ...
        results = search_web(query, num_results=results_per_query, settings=settings)
        all_results.extend(results)
        
        # Add delay between queries to avoid rate limiting
        if idx < len(queries) - 1 and delay_between_queries > 0:
            logger.debug(f"Waiting {delay_between_queries}s before next query...")
            time.sleep(delay_between_queries)
    
    logger.info(f"Total results from {len(queries)} queries: {len(all_results)}")
